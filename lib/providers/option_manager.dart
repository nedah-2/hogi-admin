  ...
  Future<void> fetchOptions() async {
    ...
    countControllers = List.generate(
        ...
        (index) => TextEditingController(
            text: formatAmount(options[index].count.toString())));
    priceControllers = List.generate(
        options.length,
        (index) => TextEditingController(
            text: formatAmount(options[index].price.toString())));
    notifyListeners();
  }
  ...
  Future<void> saveOptions() async {
    ...
    await ref.set([
      for (int i = 0; i < countControllers.length; i++)
        Option(
                count: int.parse(countControllers[i].text.trim()),
                price: int.parse(
                    priceControllers[i].text.trim().replaceAll(',', '')))
            .toJson()
    ]);
